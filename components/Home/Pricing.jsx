    ...
    >
      <div className="max-w-[1000px] mx-auto">
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-[30px] md:text-[44px] font-[800] tracking-tight mb-3 text-black">Pricing</h2>
        </motion.div>
          ...
          {plans.map((plan, index) => (
            ...
            >
              ...
              <div className={`text-center ${plan.popular ? "pt-12 pb-6 px-6" : "p-6"}`}>
                {plan.name !== "Free" && (
                  <h3 className={`text-2xl font-[800] mb-2 ${plan.popular ? "text-white" : "text-black"}`}>
                    {plan.name}
                    {/* {plan.comingSoon && <span className="text-sm ml-2 opacity-70">(Coming Soon)</span>} */}
                  </h3>
                )}
                ...
                <div className="mb-3">
                  <span className={`text-4xl font-[800] ${plan.popular ? "text-white" : "text-black"}`}>
                    {formatPrice(plan)}
                  </span>
                </div>
              ...
              <div className="px-6 pb-6">
                <div className="space-y-3 mb-6">
                  {Object.entries(plan.features).map(([key, value]) => (
                    ...
                    >
                      <span className={`font-[600] text-sm ${plan.popular ? "text-white" : "text-black"}`}>
                        {featureLabels[key]}:
                      </span>
                      <span className={`font-[500] text-sm ${plan.popular ? "text-white" : "text-[#4a5568]"}`}>
                ...
                >
                  {plan.comingSoon ? "Coming Soon" : plan.monthlyPrice === 0 ? "Get Started" : "Choose Plan"}
                </motion.button>
              </div>
            </motion.div>
          ))}
        ...
        >
          <div className="bg-white border-2 border-gray-200 rounded-[12px] overflow-hidden">
              ...
              {plans.map((plan, index) => (
                ...
                >
                  {plan.popular && (
                    ...
                    <h3 className={`text-[24px] leading-[1] font-[800] ${plan.popular ? "text-white" : "text-black"}`}>
                      {plan.name}
                    </h3>
                 ...
                 {plan.name == "Premium" && <div className="">
                    <span
                      className={`text-[12px] leading-[1] font-[800] ${plan.popular ? "text-white" : "text-black"}`}
                    >
                      {formatPrice(plan)}
                    </span>
                  </div>}
            ...
            {Object.entries(featureLabels).map(([key, label], rowIndex) => (
              ...
              >
                <div className="p-4 font-[700] text-black text-[16px] leading-[1] bg-gray-50">{label}</div>
                {plans.map((plan) => (
                  <div
                    key={`${plan.name}-${key}`}
            ...
            >
              ...
              {plans.map((plan) => (
                <div
                  ...
                  className={`p-4 text-center border-l border-gray-200 ${plan.popular ? "bg-gray-50" : "bg-white"}`}
                >
                  <motion.button
                    className={`w-full py-2 px-4 rounded-[5px] font-[500] text-sm transition-all duration-300 ${
                      plan.popular ? "bg-black text-white hover:bg-gray-800" : "bg-black text-white hover:bg-black/90"
                    } ${plan.comingSoon ? "opacity-50 cursor-not-allowed" : ""}`}
                    disabled={plan.comingSoon}
                    whileHover={!plan.comingSoon ? { scale: 1.02 } : {}}
                    whileTap={!plan.comingSoon ? { scale: 0.98 } : {}}
